...
int s3cfb_map_video_memory(struct s3cfb_global *fbdev, struct fb_info *fb)
{
	...
	fix->smem_start = (dma_addr_t)cma_alloc
		(fbdev->dev, CMA_REGION_VIDEO, (size_t)PAGE_ALIGN(fix->smem_len), 0);
...
static void s3c_fb_fence_wait(struct s3cfb_global *fbdev, struct sync_fence *fence)
{
...
void s3c_fb_update_regs(struct s3cfb_global *fbdev, struct s3c_reg_data *regs)
{
	...
	for (i = 0; i < pdata->nr_wins; i++) {
		if (regs->fence[i]) {
			s3c_fb_fence_wait(fbdev, regs->fence[i]);
		}
	}

	for (i = 0; i < pdata->nr_wins; i++) {
		if (regs->state[i] != REG_STATE_UPDATED)
	...
	for (i = 0; i < pdata->nr_wins; i++) {
		...
		if (win->enabled) {
			/* Register value of some window changed,
			 * let's check buffer start address after VSYNC event */
			reg_updated++;
		}
	}
...
#ifdef CONFIG_FB_S5P_SYSMMU
	if ((fbdev->sysmmu.enabled == false) && (fbdev->sysmmu.pgd)) {
...
#if defined(CONFIG_CPU_EXYNOS4212) || defined(CONFIG_CPU_EXYNOS4412)
static int s3c_fb_set_win_offset(unsigned int yoffset, struct fb_info *fb)
{
	struct s3cfb_window *win = fb->par;
	struct s3cfb_global *fbdev = get_fimd_global(win->id);
	struct s3c_platform_fb *pdata = to_fb_plat(fbdev->dev);
	...
	if (unlikely((fbdev->system_state == POWER_OFF) ||
				 (fbdev->regs == 0))) {
		dev_dbg(fbdev->dev, "system_state is POWER_OFF\n");
		return 0;
	}
#endif

	if (unlikely((yoffset + fb->var.yres) > fb->var.yres_virtual)) {
		dev_err(fbdev->dev, "invalid yoffset value\n");
		return -EINVAL;
	}
		...
		struct s3c_fb_win_config_data *win_data)
{
	...
	for (i = 0; i < pdata->nr_wins && !ret; i++) {
		...
		if (config->fence_fd >= 0) {
			regs->fence[i] = sync_fence_fdget(config->fence_fd);
			if (!regs->fence[i]) {
				dev_err(fbdev->dev, "failed to import fence fd %d\n", config->fence_fd);
			}
		}

		switch (config->state) {
		case S3C_FB_WIN_STATE_DISABLED:
			regs->state[i] = REG_STATE_UPDATED;	/* Skip update */
			break;
		...
		case S3C_FB_WIN_STATE_BUFFER:
			...
			break;
		case S3C_FB_WIN_STATE_OFFSET:
			s3c_fb_set_win_offset(config->offset, fb);
			regs->state[i] = REG_STATE_UPDATED;	/* Already updated */
			break;
...
#endif /* End of CONFIG_CPU_EXYNOS4x12 */
...
int s3cfb_ioctl(struct fb_info *fb, unsigned int cmd, unsigned long arg)
{
	...
	case S3CFB_WIN_BUFFER_N:
		fbdev->notify = 1;
	case S3CFB_WIN_BUFFER:
		s3c_fb_set_win_offset((unsigned int)argp, fb);
		/* Continue to wait vsync */
	...
	case S3CFB_SUPPORT_FENCE:
		if (copy_from_user(&support_fence, (unsigned int *)arg, sizeof(unsigned int)))
			...
			dev_dbg(fbdev->dev, "support_fence %d\n", fbdev->support_fence);
		}
	}
