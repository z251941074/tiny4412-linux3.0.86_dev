...
/* Support for arm-linux-gcc 4.5.1 */
extern inline struct s3cfb_extdsp_global *get_extdsp_global(int id);
#else
/* See s3cfb_extdsp_main.c for more */
extern struct s3cfb_extdsp_extdsp_desc *fbextdsp;

static inline struct s3cfb_extdsp_global *get_extdsp_global(int id) {
	return fbextdsp->fbdev[0];
}
#endif
